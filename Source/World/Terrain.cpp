    ...
    DirectX::XMFLOAT3 centerPos)
{
	...
	if (!m_TerrainGenCB.Init(pDevice.Get(), pPool, sizeof(CbTerrainGen), terrainGenCbSlots))
		return false;
	...
	for (uint32_t cz = 0; cz < kTerrainChunkDiv; ++cz)
	{
		for (uint32_t cx = 0; cx < kTerrainChunkDiv; ++cx)
		{
			...
			if (!CreateVertexBufferForChunk(pDevice.Get(), kLodSizes[0], kLodSizes[0], chunk.get()))
				return false;

			for (uint32_t lod = 0; lod < kTerrainLodLevels; ++lod)
			{
				auto indices = CreateIndices(kLodSizes[lod], kLodSizes[lod]);
				if (!chunk->m_IndexBuffers[lod].Init(pDevice.Get(), indices.size(), indices.data()))
					return false;
	...
	if (!m_MaterialCB.Init(pDevice.Get(), pPool, sizeof(CbMaterial)))
		return false;
	if (!m_Material.Init(pDevice.Get(), pPool, sizeof(CbMaterial), 1))
		return false;
...
void Terrain::Term()
{
	...
	{
		if (m_HeightMapSRV)
			pPool->FreeHandle(m_HeightMapSRV);
		if (m_VertexBufferUAV)
			pPool->FreeHandle(m_VertexBufferUAV);
	}
	...
	for (auto& chunk : m_Chunks)
	{
		if (!chunk)
			continue;
		for (uint32_t lod = 0; lod < kTerrainLodLevels; ++lod)
			chunk->m_IndexBuffers[lod].Term();
		chunk->m_VertexBuffer.Reset();
		if (chunk->m_VertexBufferUAV && pPool)
			pPool->FreeHandle(chunk->m_VertexBufferUAV);
	}
	m_Chunks.clear();
	...
	if (m_FieldMapTexture)
		m_FieldMapTexture->Term();
	if (m_MacroTexture)
		m_MacroTexture->Term();
...
void Terrain::Update(float deltaTime)
{
	...
	for (auto& chunk : m_Chunks)
	{
		...
		if (dist >= kLodDistanceMid)
			chunk->m_TargetLOD = 2; // 最も粗い
		else if (dist >= kLodDistanceNear)
			chunk->m_TargetLOD = 1; // 中間
		else
			chunk->m_TargetLOD = 0; // 最も精細
	}
}
...
void Terrain::Draw(const RenderContext& context)
{
	...
	if (!resourceManager.GetRenderSystem()->SetPipelineState(context, m_PipelineName))
		return;
	...
	{
		if (h.ptr)
			pCmdList->SetGraphicsRootDescriptorTable(static_cast<UINT>(idx), h);
	};
	...
	if (m_FieldMapTexture)
		bindTex(RootParamIndex::FieldMap, m_FieldMapTexture->GetHandleGPU());
	if (m_MacroTexture)
		bindTex(RootParamIndex::MacroTexture, m_MacroTexture->GetHandleGPU());

	// 地形テクスチャのマルチテクスチャバインド
	auto itGrass = m_TerrainTextures.find(TerrainTextureType::Grass);
	if (itGrass != m_TerrainTextures.end())
		bindTex(RootParamIndex::GrassTexture, itGrass->second->GetHandleGPU());
	auto itRock = m_TerrainTextures.find(TerrainTextureType::Rock);
	if (itRock != m_TerrainTextures.end())
		bindTex(RootParamIndex::RockTexture, itRock->second->GetHandleGPU());

	// 各チャンクのインデックスバッファを用いた描画コール
	for (const auto& chunk : m_Chunks)
	{
		// 999（未生成）の場合はスキップ
		if (chunk->m_CurrentLOD == kLodUninitialized)
			continue;
...
{
	if (m_MacroTexture)
		return m_MacroTexture->GetHandleGPU();
...
{
	if (m_HeightMapSRV)
		return m_HeightMapSRV->HandleGPU;
	return D3D12_GPU_DESCRIPTOR_HANDLE{0};
}
...
{
	if (m_FieldMapTexture)
		return m_FieldMapTexture->GetHandleGPU();
	return D3D12_GPU_DESCRIPTOR_HANDLE{0};
}
...
bool Terrain::CreateVertexBufferForChunk(
    ...
    TerrainChunk* pOut)
{
	if (!pOut || gridWidth == 0 || gridHeight == 0)
		return false;
	...
	HRESULT hr = pDevice->CreateCommittedResource(
	    ...
	    D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, // 頂点バッファとして使用する初期ステート
	    nullptr,
	    IID_PPV_ARGS(pOut->m_VertexBuffer.ReleaseAndGetAddressOf()));
	if (FAILED(hr))
	{
		ELOG("CreateCommittedResource(Chunk VB) failed. hr=0x%x", hr);
		return false;
	}
