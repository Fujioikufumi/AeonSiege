...
bool MeshRenderer::Load(const std::wstring& filePath, const std::wstring& pipelineName)
{
	...
	if (!m_MeshCB.Init(device.Get(), pool, sizeof(CbMesh), kFrameCount))
	{
...
void MeshRenderer::Draw(const RenderContext& context)
{
	...
	auto meshCB = m_MeshCB.GetPtr<CbMesh>(context.frameIndex);
	if (meshCB != nullptr)
	{
		XMStoreFloat4x4(&meshCB->World, XMMatrixTranspose(worldMatrix));
	}

	// MeshCBのハンドルを取得（現在のフレームスロット）
	D3D12_GPU_DESCRIPTOR_HANDLE meshCBHandle = m_MeshCB.GetHandleGPU(context.frameIndex);
	...
	if (m_HasAnimation && m_GameObject != nullptr)
	{
		...
		{
			// 現在のフレームスロットへボーン行列をアップロードしてからバインド
			animationController->UpdateConstantBuffer(context.frameIndex);
			boneMatrixCBHandle = animationController->GetBoneMatrixCBHandle(context.frameIndex);
		}
	}
