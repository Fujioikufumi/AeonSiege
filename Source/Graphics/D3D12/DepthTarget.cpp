...
DepthTarget::DepthTarget()
    : m_pTarget(nullptr), m_pHandleDSV(nullptr), m_pHandleSRV(nullptr), m_pPoolDSV(nullptr), m_pPoolSRV(nullptr)
{ /* DO_NOTHING */
}
    ...
    uint8_t clearStencil)
{
	...
	if (m_pHandleSRV != nullptr)
	{
		// 深度バッファのSRVには、D32_FLOATをR32_FLOATに変換する必要がある
		...
		// その他の深度フォーマットも必要に応じて変換

		m_SRVDesc.Format                        = srvFormat;
