...
//-----------------------------------------------------------------------------
Texture::Texture()
    : m_Texture(nullptr), m_Handle(nullptr), m_Pool(nullptr)
{ /* DO_NOTHING */
}
    ...
    DirectX::ResourceUploadBatch& batch)
{
	...
	HRESULT hrlt = CoInitializeEx(nullptr, COINIT_MULTITHREADED);
	if (FAILED(hrlt) && hrlt != RPC_E_CHANGED_MODE && hrlt != S_FALSE)
	{
		ELOG("Error : CoInitializeEx() Failed. retcode = 0x%x", hrlt);
		return false;
	}
...
D3D12_SHADER_RESOURCE_VIEW_DESC Texture::GetViewDesc(bool isCube) const
{
	...
	case D3D12_RESOURCE_DIMENSION_TEXTURE2D:
	{
		...
		else
		{
			...
			{
				// 2D テクスチャ（ミップあり・なしどちらも TEXTURE2D で全ミップを参照）
				viewDesc.ViewDimension                 = D3D12_SRV_DIMENSION_TEXTURE2D;
				viewDesc.Texture2D.MostDetailedMip     = 0;
				viewDesc.Texture2D.MipLevels           = desc.MipLevels; // 0 のときは「全段」の意味になる場合あり。要確認。
