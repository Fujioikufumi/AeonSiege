...
void ResultHud::SetVisible(bool visible)
{
	...
	if (m_pNumDamage)
		m_pNumDamage->SetAlpha(alpha);
	if (m_pNumKills)
		m_pNumKills->SetAlpha(alpha);
	if (m_pNumPhases)
		m_pNumPhases->SetAlpha(alpha);
}
...
void ResultHud::Update(float deltaTime)
{
	...
	if (m_pNumDamage)
		m_pNumDamage->Update(deltaTime);
	if (m_pNumKills)
		m_pNumKills->Update(deltaTime);
	if (m_pNumPhases)
		m_pNumPhases->Update(deltaTime);
}